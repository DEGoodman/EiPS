import sys, re, itertools, operator, time

## Constraints
# - the problem is modeled like a spreadsheet, with columns of data and formulas
# - some data depends on other data according to formulas.
#     when data changes, the dependent data also changes.
...
non_stop_words = [
    (),
    lambda: list(map(lambda w: w if w not in stop_words[0] else "", all_words[0])),
]
...
stop_words[0] = set(open("../static/stop_words.txt").read().split(","))
