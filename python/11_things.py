...
class DataStorageManager(TFExercise):
    ...
    def info(self):
        return super(DataStorageManager, self).info() + ": My\
            major data structure is a " + self._data.__class__.__name__
...
class StopWordManager(TFExercise):
    ...
    def info(self):
        return super(StopWordManager, self).info() + ": My major\
            data structure is a " + self._stop_words.__class__.__name__
...
class WordFrequencyManager(TFExercise):
    ...
    def sorted(self):
        return sorted(self._word_freqs.items(), key=operator.\
                        itemgetter(1), reverse=True)

    def info(self):
        return super(WordFrequencyManager, self).info() + ": My\
            major data structure is a " + self._word_freqs.\
            __class__.__name__
