...
# lead the list of stop words
f = open('../static/stop_words.txt')
...
# Part 2
# Now we need to find the 25 most frequently ocurring words.
...
while True:
    ...
    if data[25] == '': # EOF
        break
    data[26] = int(data[25].split(',')[1]) # read it as an integer
    data[25] = data[25].split(',')[0].strip() # word
    # Check if this work has more counts than the ones in memory
...
for tf in data[0:25]: # elimination of symbol tf is an exercise
    if len(tf) == 2:
        print(tf[0], '-', tf[1])
# we're done
word_freqs.close()