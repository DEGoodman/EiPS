import sys, re, operator, string, traceback, time

## Constraints
# - Every function checks its args and refuses to continue on unexpected
# - All code blocks check for errors, log context-specific messages on error,
#     and pass errors up the function call chain
...
def extract_words(path_to_file):
    ...
    try:
        ...
        print(
            "I/O error({0}) when opening {1}: {2}! I quit!".format(
...
def remove_stop_words(word_list):
    ...
    try:
        ...
        print(
            "I/O error({0}) when opening ../static/stops_words.txt: {1}! I quit!".format(
                e.errno, e.strerror
            )
        )
        raise e
